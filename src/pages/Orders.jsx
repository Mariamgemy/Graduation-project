...
import { Container, Row, Col, Card, Alert, Badge, Form, Button, Modal } from "react-bootstrap";
...
const Orders = () => {
  ...
  const fetchCivilRequests = async () => {
    try {
      ...
      const response = await fetch(
        `https://smartgovernment-fpcxb3cmfef3e6c0.uaenorth-01.azurewebsites.net/api/CivilDocuments/my-requests?userNID=${user.nationalId || user.nid}`,
        {
          method: 'GET',
          headers: {
  ...
  return (
    <Container className="mt-5 pt-5">
      ...
      {/* قسم الأحوال المدنية */}
      <div className="civil-services-section">
        <h3 className="mb-4">
          <FaFileAlt className="me-2" />
          خدمات الأحوال المدنية
        </h3>

        {/* بحث عن طلب بالرقم المرجعي */}
        <Card className="mb-4">
          <Card.Header>
            <h5 className="mb-0">
        ...
        <Card>
          <Card.Header className="d-flex justify-content-between align-items-center">
            ...
            <Button variant="outline-primary" size="sm" onClick={fetchCivilRequests}>
              تحديث
            </Button>
          </Card.Header>
          <Card.Body>
            {civilRequests.length === 0 ? (
              ...
              <div className="row">
                {civilRequests.map((request, index) => (
                  <div key={request.id || index} className="col-md-6 col-lg-4 mb-3">
                    <Card>
                      <Card.Header>
                      ...
                      <Card.Body>
                        {request.referenceNumber && (
                          <p>
                            <strong>الرقم المرجعي:</strong> {request.referenceNumber}
                          </p>
                        )}
                        <p>
                        ...
                        {request.estimatedDeliveryDate && (
                          <p>
                            ...
                            {new Date(request.estimatedDeliveryDate).toLocaleDateString("ar-EG")}
                          </p>
                        )}
                        <div className="mt-3">
                          <Button
                            ...
                            size="sm"
                            onClick={() => trackRequest(request.id || request.referenceNumber)}
                            disabled={trackingLoading}
                          >
      ...
      <Modal show={showTrackingModal} onHide={() => setShowTrackingModal(false)} size="lg">
        ...
        <Modal.Body>
          {trackingResult && (
            <div>
              <div className="mb-3">
                <strong>حالة الطلب:</strong> {getStatusBadge(trackingResult.status)}
              </div>
              
              {trackingResult.documentType && (
                <p><strong>نوع الوثيقة:</strong> {getDocumentTypeText(trackingResult.documentType)}</p>
              )}
              
              {trackingResult.ownerName && (
                <p><strong>اسم صاحب الوثيقة:</strong> {trackingResult.ownerName}</p>
              )}
              
              {trackingResult.referenceNumber && (
                <p><strong>الرقم المرجعي:</strong> {trackingResult.referenceNumber}</p>
              )}
              
              {trackingResult.createdAt && (
                <p><strong>تاريخ الطلب:</strong> {new Date(trackingResult.createdAt).toLocaleDateString("ar-EG")}</p>
              )}
              
              {trackingResult.estimatedDeliveryDate && (
                <p><strong>تاريخ التسليم المتوقع:</strong> {new Date(trackingResult.estimatedDeliveryDate).toLocaleDateString("ar-EG")}</p>
              )}
                    ...
                    {trackingResult.statusHistory.map((history, index) => (
                      <div key={index} className="d-flex justify-content-between align-items-center p-2 border-bottom">
                        ...
                        <small className="text-muted">
                          {new Date(history.timestamp).toLocaleDateString("ar-EG")}
                        </small>
                      </div>
                    ))}
                  </div>
